...
const DataPickerPhp = () => {
    ...
    useEffect(() => {
        const fetchGroups = async () => {
            try {
                const res = await fetch("http://localhost/data/groups.php");
    ...
    const handlePersonChange = async (e) => {
        ...
        if (name === "profileName") {
            ...
            try {
                const res = await fetch(`http://localhost/data/persons.php?q=${encodeURIComponent(value)}`);
                const data = await res.json();
                if (data.success) {
    ...
    const selectPerson = async (person) => {
        ...
        try {
            const res = await fetch(`http://localhost/data/person-groups.php?personId=${person.id}`);
            const data = await res.json();
            if (data.success) setPersonGroups(data.groups);
        } catch (err) {
            console.error("Failed to fetch person groups", err);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            try {
                const response = await fetch("http://localhost/data/save-data.php", {
                    method: "POST",
                    headers: {
    ...
    const saveGroup = async () => {
        if (!groupInfo.groupName.trim()) return alert("Group name is required");
        if (!personDbId) return alert("Please select a person first");
    ...
    const savePost = async () => {
        ...
        try {
            const res = await fetch("http://localhost/data/posts.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    personId: personDbId,
    ...
    const handleGroupSelection = async (g) => {
        setGroupInfo({
            id: g.id,
        ...
        try {
            const res = await fetch(`http://localhost/data/posts.php?personId=${personDbId}&groupId=${g.id}`);
            const data = await res.json();
            setGroupRecords(data.success ? data.posts : []);
        } catch (err) {
                                                ...
                                                {personGroups.map(g => (
                                                    <div
                                                        ...
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                // Load posts for a selected group
const res = await fetch(`http://localhost/data/posts.php?personId=${personDbId}&groupId=${g.id}`);
